...
fName = "LA_SPEZIA_RADIAZIONE_SOLARE__RADIAZIONE_SOLARE_GIORNALIERA___Jcm2.csv"
#fName = "MONTE_ROCCHETTA_RADIAZIONE_SOLARE__RADIAZIONE_SOLARE_GIORNALIERA___Jcm2.csv"

#dateRef = datetime(2010,10,1)
dateRef = datetime(2010,1,1)
...
#    print(row)
    if "2012" in row["Inizio rilevazione"]: continue
...
#    days = (int(date.timestamp()) - int(datetime(2010,1,1).timestamp()))/86400 #3.154e+7
    days = (date - dateRef).days
    temp = float(row["RADIAZIONE_SOLARE__RADIAZIONE_SOLARE_GIORNALIERA___Jcm2"])
...
#    average[x] = sum(average[x])/len(average[x])
    max_[x] = max(average[x])
    average[x] = (sum(average[x])/len(average[x]))/max_[x]
#    average[x] = max(average[x])
...
function_average = ROOT.TF1("function","[0] + [1]*sin([2]*x + [3]) + [4]*sin([5]*x + [6])",graph_average.GetXaxis().GetXmin(),graph_average.GetXaxis().GetXmax())
...
#        graphs[year] = ROOT.TGraph(str(year),str(year),366)
    days = x - int((datetime(year,1,1)-date2010).days)
    print(year, days, ys[i])
#    graphs[year].AddPoint(days, ys[i])
    graphs[year].AddPoint(days, ys[i]/max_[days%365])
